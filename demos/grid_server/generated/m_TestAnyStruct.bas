Attribute VB_Name = "m_TestAnyStruct"
...
Public Const TypeId As String = "IDL:TestAnyStruct:1.0"
...
Public Function TypeCode() As cOrbTypeCode
    ...
    If oRecTC Is Nothing Then
        ...
        oMemSeq.Item(0).name = "string_value"
        Set oMemSeq.Item(0).p_type = oOrb.createStringTc(20)
        Set oMemSeq.Item(1) = New cCBStructMember
        oMemSeq.Item(1).name = "long_value"
        Set oMemSeq.Item(1).p_type = oOrb.createPrimitiveTc(3) 'VBOrb.TCLong
        'Overwrite place holder
        Call oRecTC.setRecTc2StructTc("TestAnyStruct", oMemSeq)
    End If
    Set TypeCode = oRecTC
    Exit Function
ErrRollback:
...
Public Function extractFromAny(ByVal oAny As cOrbAny) As c_TestAnyStruct
    Dim oStruct As c_TestAnyStruct
    Set oStruct = New c_TestAnyStruct
    Call oStruct.initByAny(oAny)
    Set extractFromAny = oStruct
End Function

'Helper, struct.writeMe() -> oAny.initByReadValue()
Public Function cloneAsAny(ByVal oStruct As c_TestAnyStruct) As cOrbAny
    On Error GoTo ErrHandler
    Dim oAny As cOrbAny
