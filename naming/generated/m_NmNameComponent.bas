Attribute VB_Name = "m_NmNameComponent"
...
Public Const TypeId As String = "IDL:omg.org/CosNaming/NameComponent:1.0"
...
Public Function TypeCode() As cOrbTypeCode
    ...
    If oRecTC Is Nothing Then
        ...
        oMemSeq.Item(0).name = "id"
        Set oMemSeq.Item(0).p_type = oOrb.createStringTc(0)
        Set oMemSeq.Item(0).p_type = oOrb.createAliasTc("IDL:omg.org/CosNaming/Istring:1.0", "Istring", oMemSeq.Item(0).p_type)
        Set oMemSeq.Item(1) = New cCBStructMember
        oMemSeq.Item(1).name = "kind"
        Set oMemSeq.Item(1).p_type = oOrb.createStringTc(0)
        Set oMemSeq.Item(1).p_type = oOrb.createAliasTc("IDL:omg.org/CosNaming/Istring:1.0", "Istring", oMemSeq.Item(1).p_type)
        'Overwrite place holder
        Call oRecTC.setRecTc2StructTc("NameComponent", oMemSeq)
    End If
    Set TypeCode = oRecTC
    Exit Function
ErrRollback:
...
Public Function extractFromAny(ByVal oAny As cOrbAny) As c_NmNameComponent
    Dim oStruct As c_NmNameComponent
    Set oStruct = New c_NmNameComponent
    Call oStruct.initByAny(oAny)
    Set extractFromAny = oStruct
End Function

'Helper, struct.writeMe() -> oAny.initByReadValue()
Public Function cloneAsAny(ByVal oStruct As c_NmNameComponent) As cOrbAny
    On Error GoTo ErrHandler
    Dim oAny As cOrbAny
